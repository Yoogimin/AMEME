...
<body>
	<form action="mModify" method="post" enctype="multipart/form-data">
		<table>
			...
			<!-- 비밀번호와 확인의 입력값이 일치할 경우 "비밀번호 일치"(파란색) -->
			...
			<tr>
				...
				<c:choose>
				    ...
				    <td>남자 <input type="radio" name="mGender" value="남자" checked /> 
				        여자 <input type="radio" name="mGender" value="여자" />
				    </td>
				    </c:when>
				    
				    <c:otherwise>
				    <td>남자 <input type="radio" name="mGender" value="남자" /> 
			...
			<tr>
				...
				<img
				id="preview" width="150px" src="./resources/profile/${modify.MProfileName}" />
...
<script>
	...
	*/
	...
	mPw.keyup(()=>{
		...
		let eng = pwVal.search(/[a-z]/);	// 영어 입력 여부 : 입력하면 -1이 아니다
		...
		// (1) 공백x : "공백없이 입력"
		// (2) 비밀번호 8~16자리 : 만족하지 못할 경우 "비밀번호는 8자리에서 16자리로 입력"
