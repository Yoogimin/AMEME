...
<style>
table {
    border-collapse: collapse; /* 테이블의 경계를 병합하도록 설정 */
    border: 1px solid black; /* 테두리 색상과 스타일 설정 */
    width: 90%; /* 테이블의 너비 설정 */
    font-family: Arial, sans-serif; /* 폰트 설정 */
    border-radius: 1px; /* 테이블 외각선 둥글게 설정 */
    border-spacing: 0; /* 셀 사이의 간격을 없앰 */
}
...
th {
    background-color: lightgray; /* 테이블 헤더 배경색 설정 */
}

td {
    background-color: white; /* 테이블 셀 배경색 설정 */
}
</style>
...
<body class="g-sidenav-show   bg-gray-100">
	...
	<main class="main-content position-relative border-radius-lg ">
		...
		<!-- End Navbar -->
		<h3 style="color: white;">현재페이지 : ${paging.page}</h3>

		<table>
			<caption></caption>
			<tr>
				<td colspan="4" align="right" style="padding: 10px;">
					<form action="bSearch" method="GET" style="display: inline-block;">
						<!-- 검색 카테고리 -->
						...
						<!-- 검색어 -->
						<input type="text" name="keyword" style="margin-right: 10px;">

						<!-- 검색버튼 -->
						<input type="submit" value="검색">

					</form> <select id="limit" style="margin-left: 10px;">
				...
				<th colspan="4">
					<nav aria-label="..." >
								<ul class="pagination" style="margin : 0 auto; width : 300px;">
									...
									<c:if test="${paging.page > 1}">
										<li class="page-item">
											<a class="page-link" href="bList?page=${paging.page-1}&limit=${paging.limit}">이전</a>
										</li>
									</c:if> 
									...
									<c:forEach var="i" begin="${paging.startPage}" end="${paging.endPage}">
										...
										<c:if test="${paging.page != i}">
											<li class="page-item">
												<a class="page-link" href="bList?page=${i}&limit=${paging.limit}">${i}</a>
											</li>
										</c:if>
									...
									<c:if test="${paging.page < paging.maxPage}">
										<li class="page-item">
											<a class="page-link" href="bList?page=${paging.page+1}&limit=${paging.limit}">다음</a>
										</li>
									</c:if> 
								</ul>
							</nav>
